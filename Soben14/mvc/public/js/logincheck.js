function checkFields() {
  ...
  if (!(validUserName && userName.length >= 1 && userName.length <= 20)) {
    ...
    document.getElementById("errorDiv").innerHTML = "Username must only consist of letters, numbers and be less than 20 chars."
    return false;
  }

  if (!(password.length >= 8 && password.length <= 30 && validPassword)) {
    console.log("Your password must be atleast 8 characters long, have atleast one upper case"
    + ", one lower case and one number");
    document.getElementById("errorDiv").innerHTML = "Password must contain 8-30 characters, have atleast one upper case"
    + ", one lower case and one number."
    return false;
  }
